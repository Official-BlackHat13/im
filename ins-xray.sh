...
wget -qO /usr/local/etc/xray/config.json "https://raw.githubusercontent.com/syndrexhub/im/main/xray_tls_ws_mix-rprx-direct.json"
wget -qO /etc/nginx/conf.d/${domain}.conf "https://raw.githubusercontent.com/syndrexhub/im/main/web.conf"

sed -i "s/xxx/${domain}/g" /etc/nginx/conf.d/${domain}.conf
sed -i "s/x.x.x.x/${ip}/g" /etc/nginx/conf.d/${domain}.conf

wget -qO web.tar.gz "https://github.com/syndrexhub/im/raw/main/web.tar.gz"
rm -rf /var/www/html/*
